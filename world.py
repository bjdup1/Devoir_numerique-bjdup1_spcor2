...
class World:
    ...
    def compute(self, nb_relaxation_iterations: int = 1000):
        ...
        if not self.wires:
            ...
            self._energy_flux = VectorField((1/mu_0) * self._electric_field.cross(self._magnetic_field))
    ...
    def show_wires_voltage(self):
        ...
        """
        if self.wires:
            self._wires_voltage.show(title="Initial voltage [V]", x_label = 'x [m]', y_label = 'y [m]')
        else:
    ...
    def show_potential(self):
        ...
        """
        if self.wires:
            self._potential.show(title="Potential [V]", x_label = 'x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException
    ...
    def show_electric_field(self, hide_wires: bool = True):
        ...
        if self.wires:
            ...
            electric_field.show(title="Electric field [V/m]", x_label = 'x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException
    ...
    def show_magnetic_field(self):
        ...
        """
        if self.wires:
            self._magnetic_field.z.show(title="Magnetic field (z component) [T]", x_label = 'x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException
    ...
    def show_energy_flux(self):
        ...
        """
        if self.wires:
            self._energy_flux.show(title="Energy flux [W/m$^2$]", x_label = 'x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException
